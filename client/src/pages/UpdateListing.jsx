...
export default function CreateListing() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        ...
        // ✅ Set uploaded images (fix for missing images)
        setUploadedImages(propertyData.imageUrls || []);
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedListing = {
      ...formData,
      imageUrls: uploadedImages, // ✅ Ensure correct key
      userId: currentUser._id, // ✅ Ensure user ID is sent
    };
  
    try {
      const res = await fetch(
        `http://localhost:5000/properties/updateproperty/${params.listingId}`,
        {
          method: "PUT",
          headers: {
